...
boost::shared_ptr<l1t::CaloParams> L1TCaloParamsStage1OnlineProd::newObject(const std::string& objectKey) {
    ...
    if (!pp_algo_regs_key.empty()) {
        ...
        caloParams->setJetSeedThreshold(jet_seed_thr/2.);
        caloParams->setEtSumEtThreshold(0, ht_sum_thr/2.);
        caloParams->setTauSeedThreshold(tau_seed_thr/2.);
        caloParams->setTauNeighbourThreshold(tau_neighbour_thr/2.);
        caloParams->setTauMaxPtTauVeto(tau_max_pt_veto/2.);
        
        int minTauEta, maxTauEta;
        try {
            computeTauRangeEtaFromColumnEn(iso_tau_column_en, minTauEta, maxTauEta);
            caloParams->setIsoTauEtaMin(minTauEta);
            caloParams->setIsoTauEtaMax(maxTauEta);
    ...
    if (!pileup_sub_lut_key.empty()) {
        ...
        if (energy_thr == 0) {
            ...
            caloParams->setRegionPUSType("PUM0");
            caloParams->setRegionPUSParams(pumVector);
        } else {
    ...
    if (!eg_iso_lut_key.empty()) {
        ...
        l1t::LUT lut(is);
        caloParams->setEgIsolationLUT(lut);
    } else {
        // No LUT to load
    }
    ...
    if (!tau_iso_lut_key.empty()) {
        ...
        std::istringstream is(oss.str());
        l1t::LUT lut(is);
        caloParams->setTauIsolationLUT(lut);
    } else {
        // No LUT to load
    }
    ...
    if (!tau_calib_lut_key.empty()) {
        ...
        std::istringstream is(oss.str());
        l1t::LUT lut(is);
        caloParams->setTauCalibrationLUT(lut);
    } else {
        // No LUT to load
    }
    ...
    if (!tau_fiber_lut_key.empty()) {
        ...
        std::istringstream is(oss.str());
        l1t::LUT lut(is);
        caloParams->setTauEtToHFRingEtLUT(lut);
    } else {
        // No LUT to load
    }
    ...
    if (!jet_calib_lut_key.empty()) {
        ...
        std::istringstream is(oss.str());
        l1t::LUT lut(is);
        caloParams->setJetCalibrationLUT(lut);
    } else {
        // No LUT to load
    }
    ...
    if ((etSumEtaMin.size() == etSumEtaMax.size()) &&  (etSumEtaMin.size() == etSumEtThreshold.size())) {
      for (unsigned i=0; i<etSumEtaMin.size(); ++i) {
        caloParams->setEtSumEtaMin(i, etSumEtaMin.at(i));
        caloParams->setEtSumEtaMax(i, etSumEtaMax.at(i));
        caloParams->setEtSumEtThreshold(i, etSumEtThreshold.at(i));
      }
    }
    else {
      edm::LogError("l1t|calo") << "Inconsistent number of EtSum parameters" << std::endl;
    }
    caloParams->setTowerLsbSum(towerLsbSum);
    caloParams->setRegionLsb  (regionLsb);
    caloParams->setJetLsb     (jetLsb);
    caloParams->setEgLsb      (egLsb);
